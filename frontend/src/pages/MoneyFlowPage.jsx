  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <Loader2 className="animate-spin text-deep-blue" size={48} />
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-charcoal-gray flex items-center gap-3">
          <ArrowRightLeft className="text-deep-blue" size={32} />
          Money Flow
        </h1>
        <button
          onClick={() => setShowPaymentModal(true)}
          className="bg-deep-blue text-white px-6 py-2 rounded-full font-bold hover:opacity-90 transition flex items-center gap-2 shadow-lg shadow-blue-100"
        >
          <Plus size={20} />
          Record Payment
        </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <section className="lg:col-span-1 space-y-4">
          <h2 className="text-xl font-bold text-charcoal-gray px-2">Net Balances</h2>
            ...
            {balances.length > 0 ? balances.map((bal, idx) => (
              <div key={idx} className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-bold text-gray-400 uppercase tracking-wider">{bal.user_a_name} owes</span>
                  <span className="text-xl font-black text-alert-red">{bal.amount.toFixed(2)}</span>
                </div>
                <div className="flex items-center gap-2 text-charcoal-gray font-bold">
                  <span>to</span>
                  <div className="px-3 py-1 bg-blue-50 text-deep-blue rounded-full text-sm">
                    {bal.user_b_name}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-8 rounded-3xl border border-dashed border-gray-200 text-center">
                <p className="text-gray-400 italic">No outstanding balances!</p>
              </div>
            )}
          </div>
        </section>
        ...
        <section className="lg:col-span-2 space-y-4">
          <h2 className="text-xl font-bold text-charcoal-gray px-2">Payment Tracker</h2>
          <div className="space-y-3">
            {payments.length > 0 ? payments.map((p) => (
              <div key={p.payment_id} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4 group">
                <div className={`p-3 rounded-full ${p.payer_user_id === user.user_id ? 'bg-blue-50 text-deep-blue' : 'bg-green-50 text-vibrant-green'}`}>
                ...
                <div className="flex-grow">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-black text-vibrant-green">{p.amount.toFixed(2)}</span>
                  </div>
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
                    <span className="text-xs text-gray-400 flex items-center gap-1">
                      <Calendar size={12} /> {p.payment_date}
                    </span>
                    <span className="text-xs text-deep-blue/60 flex items-center gap-1 bg-blue-50 px-2 py-0.5 rounded-full font-medium">
                ...
                {(p.payer_user_id === user.user_id || p.receiver_user_id === user.user_id || user.administrator) && (
                  <button 
                    ...
                    className="p-2 text-gray-300 hover:text-alert-red transition"
                  >
                    <Trash2 size={18} />
                  </button>
                )}
          ...
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
              <h2 className="text-xl font-bold text-charcoal-gray flex items-center gap-2">
                <DollarSign size={20} className="text-deep-blue" />
                Record a Payment
              </h2>
              <button onClick={() => setShowPaymentModal(false)} className="text-gray-400 hover:text-charcoal-gray transition"><X /></button>
            </div>
            <form onSubmit={handleCreatePayment} className="p-8 space-y-6">
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-charcoal-gray mb-2">From (Sender)</label>
                  <select
                    required
                    className="w-full p-3 bg-light-gray rounded-xl outline-none focus:ring-2 focus:ring-deep-blue"
                ...
                <div>
                  <label className="block text-sm font-bold text-charcoal-gray mb-2">To (Receiver)</label>
                  <select
                    required
                    className="w-full p-3 bg-light-gray rounded-xl outline-none focus:ring-2 focus:ring-deep-blue"
                ...
                <div>
                  <label className="block text-sm font-bold text-charcoal-gray mb-2">Amount</label>
                  <input
                    required
                    type="number"
                    step="0.01"
                ...
                <div>
                  <label className="block text-sm font-bold text-charcoal-gray mb-2">Date</label>
                  <input
                    required
                    type="date"
                    className="w-full p-3 bg-light-gray rounded-xl outline-none focus:ring-2 focus:ring-deep-blue"
                ...
                <div>
                  <label className="block text-sm font-bold text-charcoal-gray mb-2">Note (Optional)</label>
                  <textarea
                    className="w-full p-3 bg-light-gray rounded-xl outline-none focus:ring-2 focus:ring-deep-blue"
              ...
              <button
                ...
                className="w-full py-4 bg-deep-blue text-white font-bold rounded-2xl hover:opacity-90 transition shadow-lg shadow-blue-100 flex items-center justify-center gap-2 disabled:opacity-50"
              >
